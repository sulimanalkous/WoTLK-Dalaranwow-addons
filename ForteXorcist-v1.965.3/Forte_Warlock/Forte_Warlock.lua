...
if FW.CLASS == "WARLOCK" then
	...
	FW:RegisterCustomName(54370,FWL.NETHER_PROTECTION_HOLY);
	FW:RegisterCustomName(54371,FWL.NETHER_PROTECTION_FIRE);
	FW:RegisterCustomName(54372,FWL.NETHER_PROTECTION_FROST);
	FW:RegisterCustomName(54373,FWL.NETHER_PROTECTION_ARCANE);
	FW:RegisterCustomName(54374,FWL.NETHER_PROTECTION_SHADOW);
	FW:RegisterCustomName(54375,FWL.NETHER_PROTECTION_NATURE);
	...
	if ST then
		...
		-- id/name, ticks total
		ST:RegisterDrain(1120,	5); -- Drain Soul
		ST:RegisterDrain(689,	5); -- Drain Life
		ST:RegisterDrain(5138,	5); -- Drain Mana
		ST:RegisterDrain(1949,	15); -- Hellfire
		ST:RegisterDrain(5740,	4); -- Rain of Fire
		...
		ST:RegisterSpell(172,	1,018,1,ST.DEFAULT);-- Corruption
			ST:RegisterSpecialRefresh(172);
			ST:RegisterHastedGlyph(172,70947); -- set to hasted dot, requires Glyph of Quick Decay
			ST:RegisterSpellModRank(172, 	1, -6);
			ST:RegisterSpellModRank(172, 	2, -3);
			ST:RegisterSpellModSetB(172,	"Voidheart Raiment", 4,  3);
		ST:RegisterSpell(30108,	1,015,1,ST.DEFAULT); -- Unstable Affliction
		ST:RegisterSpell(980,	1,024,1,ST.SHARED); -- Curse of Agony
			ST:RegisterTickSpeed(980, 2); -- set tick speed to 2 instead of 3
			ST:RegisterSpellModGlph(980, 56282, 1, 4);
		ST:RegisterSpell(1490,	1,300,0,ST.SHARED); -- Curse of the Elements
		ST:RegisterSpell(702,	1,120,0,ST.SHARED); -- Curse of Weakness
		ST:RegisterSpell(603,	1,060,0,ST.SHARED); -- Curse of Doom
		ST:RegisterSpell(1714,	1,030,0,ST.SHARED,010); -- Curse of Tongues
		ST:RegisterSpell(46434,	1,012,0,ST.SHARED); -- Curse of Exhaustion
		ST:RegisterSpell(710,	1,030,0,ST.UNIQUE,010); -- Banish
			ST:RegisterSpellModRank(710, 	1, -10);
		ST:RegisterSpell(5782,	1,020,0,ST.UNIQUE,010); -- Fear
			ST:RegisterSpellModRank(5782, 1, -10);
			ST:RegisterSpellModRank(5782, 2, -5);
		ST:RegisterSpell(5484, 0,008,0,ST.UNIQUE); -- Howl of Terror
			ST:RegisterSpellModRank(5484, 	1, -2);
		...
		ST:RegisterSpell(17962,	1,006,1,ST.DEFAULT); -- Conflagrate
	...
	local function WL_ScanBagsChilled()
		FW:RegisterThrottle(WL_ScanBags);
	end
	...
	FW:SetMainCategory(FWL.SHARD_MANAGER,FW.ICON.SOULBAG,9,"SHARDMANAGER");
		FW:SetSubCategory(FW.NIL,FW.NIL,1);
	...
	if CD then
		...
		FW.Default.Cooldown.Filter[FW:SpellName(4511)] = 		{nil,nil,{-1}};--phaseshift
