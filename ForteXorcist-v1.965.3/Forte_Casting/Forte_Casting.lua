	...
	if not CA_IsChannel[spellName] then -- if marked as special channel it's already taken care off
		...
		if ST and ST.Drain[spellName] then
			...
			if i then
				...
				ST.Tick[spellName] = realduration/ST.Drain[spellName]; -- use this instead of calculating at update
				ST.ST:insert(endTime,FW:CastTime(spell),realduration,CA_SelfQueue[i][5],0,ST.DRAIN,icon,spellName,CA_SelfQueue[i][8],0,CA_SelfQueue[i][13],0,CA_SelfQueue[i][11],0,1,0,0,ST:GetFilterType(spellName,ST.DRAIN),CA_SelfQueue[i][12],endTime,realduration,1.0,0);
			end
		end
		-- *** end new drain code ***
		if target then
...
function CA:Duration(s,rank,p,u) -- spell, rank, combopoints, unit type (2=players)
	...
	if ST and ST.Track[s] then
		...
		if ST.Track[s]["r"] and ST.Track[s]["r"][rank] then
			dura = dura + ST.Track[s]["r"][rank];
		end
