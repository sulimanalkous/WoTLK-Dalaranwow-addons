...
function TRP_Chat_DropDownMenu_OnLoad()
	...
	elseif TRP_Module_Registre[Royaume][TRPCHATNAME] then
		...
		UIDropDownMenu_AddButton(info);
		info = {};
		info.text = "Report Spam";
		info.func = function() 
			local dialog = StaticPopup_Show("CONFIRM_REPORT_SPAM_CHAT", TRPCHATNAME);
			if ( dialog ) then
				dialog.data = TRP_Chat_DropDownMenu.lineID;
			end
		end;
		UIDropDownMenu_AddButton(info);
