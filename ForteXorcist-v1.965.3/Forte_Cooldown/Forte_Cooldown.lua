...
local GetSpellTexture = GetSpellTexture; -- 4.0 GetSpellBookItemTexture
local GetSpellCooldown = GetSpellCooldown;

if FW.CATACLYSM then
	GetSpellName = GetSpellBookItemName; -- 4.0 GetSpellBookItemName
	GetSpellTexture = GetSpellBookItemTexture; -- 4.0 GetSpellBookItemTexture
end
			...
			if t14 > 0 and t17==0 then
				...
				if s.Splash and t15==-1 then
					...
					if s.SparkEnable then
						...
						--bar.splashiconspark:SetAlpha(s.Spark);
...
FW:SetMainCategory(FWL.COOLDOWN_TIMER,FW.ICON.CD,4,"COOLDOWN","Cooldown","Cooldown");
	...
	FW:SetSubCategory(FWL.TIME_RANGE,FW.ICON.TIME,4);	
		...
		FW:RegisterOption(FW.NU2,1,FW.NON,FW.DIS_COL..FWL.MIN_DURATION,	FWL.MIN_DURATION_TT,		"MinRange",		CD_CooldownShow,0);
		FW:RegisterOption(FW.NU2,1,FW.NON,FW.DIS_COL..FWL.MAX_DURATION,	FWL.MAX_DURATION_TT,		"MaxRange",		CD_CooldownShow,0);
	...
	FW:SetSubCategory(FWL.MY_COOLDOWNS,FW.ICON.FILTER,5,FW.REDUCED_ALPHA);
	...
	FW:SetSubCategory(FWL.BUFFS_DEBUFFS_ON_ME,FW.ICON.FILTER,5,FW.REDUCED_ALPHA);	
...
FW.Default.Cooldown.Filter = 	{
	--[FW:ItemName(32757)] = 	{nil,nil,{-1}}, --"Blessed Medallion of Karabor"
	--[FW:ItemName(28111)] = 	{nil,nil,{-1}}, --"Everlasting Underspore Frond"
